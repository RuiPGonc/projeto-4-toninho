export async function createNewUser(token,userInfo, password) {
  ...
  export async function createNewAdmin(token,userInfo, password) {
    try {
        ...
        console.log(userInfo.photoUrl)
      return await fetch(
        `http://localhost:8080/Projeto-iv/rest/ToDo_app/users/newAdmin`,
        {
          method: "POST",
          headers: {
            Accept: "application/json",
        ...
        .then((response) => {
            if (response.status === 200) {
                ...
                throw new Error("Erro ao criar novo Admin");
              }
        });
    } catch (error) {
      console.error(error);
